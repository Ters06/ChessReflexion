...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // IMPORTANT: We wait for main.js to create the modal, then we select the button inside it.
    ...
    async function fetchAndDisplayGames() {
        ...
        try {
            ...
            if (!response.ok) throw new Error("Impossible de récupérer la liste des parties.");
            ...
            if (games.length === 0) {
                ...
                games.forEach(game => {
                    ...
                    gameElement.className = "bg-slate-800/50 p-4 rounded-lg flex justify-between items-center border border-slate-700 hover:border-cyan-500/50 transition-colors duration-300 block";
                    
                    const playedAsText = game.played_as === 'w' ? 'Blancs' : 'Noirs';
                    const statusText = game.status === 'in_progress' ? 'En cours' : `Terminée (${game.result || 'N/A'})`;
                    const lastUpdate = new Date(game.updated_at).toLocaleDateString('fr-FR');
                    ...
                    gameElement.innerHTML = `
                        ...
                        <span class="text-cyan-400 font-semibold hover:text-cyan-300">${game.status === 'in_progress' ? 'Reprendre' : 'Analyser'} →</span>
                    `;
    ...
    // CORRECTION MAJEURE: On attache l'événement au bouton de la modale ici.
    if (startGameBtn) {
        startGameBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(error.message || "La création de la partie a échoué.");
                }
