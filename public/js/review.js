...
document.addEventListener("DOMContentLoaded", async () => {
  function waitForChessJS(callback) {
    if (typeof Chess === "function") {
      callback();
    } else {
      console.log("review.js: Attente de chess.js...");
      setTimeout(() => waitForChessJS(callback), 100);
    }
  }

  waitForChessJS(async () => {
    ...
    async function initializePage() {
      ...
      try {
        // app.js has already validated the token. We just need to load game data.
    ...
    function renderBoard(fen) {
      ...
      for (let i = 0; i < 8; i++) {
        const row =
          gameData.played_as === "w" ? rows[i] : [...rows[i]].reverse();
        for (let j = 0; j < 8; j++) {
    ...
    async function saveGameState() {
      try {
        ...
        if (!response.ok)
          throw new Error("La sauvegarde de la partie a échoué.");
      } catch (error) {
